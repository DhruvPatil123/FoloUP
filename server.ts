...
async function seedFirestoreIfNeeded() {
  try {
    ...
    if (snapshot.empty) {
      ...
      // Seed Branding
      await setDoc(doc(db, "branding", "config"), INITIAL_DATABASE.branding);
...
async function getFirestoreBranding(workspaceId: string = "sandbox"): Promise<any> {
  try {
    ...
    return {
      ...
      companyName: workspaceId === "sandbox" ? "FoloUp AI" : workspaceId.split('.')[0].toUpperCase() + " Workspace"
    };
  } catch (err) {
...
async function sendRealOrSimulatedEmail(to: string, subject: string, template: string, workspaceId: string = "sandbox"): Promise<boolean> {
  try {
    ...
    const provider = branding.emailProvider || 'simulated';
...
app.post("/api/auth/magic-link-send", async (req, res) => {
  try {
    ...
    const magicLinkUrl = `/magic-verify?token=${token}&email=${encodeURIComponent(email)}`;
...
FoloUp Identity Management System`;
    ...
    res.json({ success: true, email, token, workspaceId, magicLinkUrl });
  } catch (err: any) {
  ...
  wss.on("connection", async (clientWs) => {
      ...
      liveSession = await ai.live.connect({
        ...
        callbacks: {
          ...
          onclose: () => {
            ...
            clientWs.send(JSON.stringify({ type: "system", message: "Gemini live session closed" }));
          },
          onerror: (err: any) => {
            console.error("[Gemini Live Error]", err);
            clientWs.send(JSON.stringify({ type: "warning", message: "Live STT session warning" }));
          }
        }
      });
    ...
    async function triggerFallbackRollingTranscription() {
      ...
      try {
        ...
        const combinedBase64PCM = activeWindow.join("");

        const ai = getAiClient();
        console.log("[STT Fallback] Conducting rolling transcription on PCM buffer...");
        const response = await ai.models.generateContent({
          model: "gemini-3.5-flash",
          contents: [
            {
              inlineData: {
                data: combinedBase64PCM,
                mimeType: "audio/pcm;rate=16000"
              }
            },
            {
              text: "Perform exact, verbatim speech-to-text transcription of this audio. Output ONLY the spoken text, nothing else. If you hear silence or non-speech, output an empty response. Do not comment or discuss."
            }
          ]
        });
        ...
        if (cleanText) {
          clientWs.send(JSON.stringify({ type: "transcript", text: cleanText, isFinal: false }));
        }
      } catch (err: any) {
