...
export default function RoleSelection({ onSelectRole, campaigns, activeCampaignId }: RoleSelectionProps) {
  ...
  const handleSendMagicLink = async (e: FormEvent) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setRecruiterError(data.message || 'Failed to dispatch magic link. Please check network state.');
      }
    } catch (err) {
      setMagicLoading(false);
  ...
  const handleVerifyMagicLink = async (linkUrl: string) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setRecruiterError(data.message || 'Magic link is invalid or expired.');
      }
    } catch (err) {
      setMagicLoading(false);
  ...
  const handleSSOLogin = async (profile: typeof SSO_PROVIDERS[0]) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setRecruiterError(data.message || 'SSO authentication profile rejected.');
      }
    } catch (err) {
      setMagicLoading(false);
      ...
      <div className="md:col-span-12 lg:col-span-12 xl:col-span-5 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950 text-white p-9 flex flex-col justify-between relative overflow-hidden lg:p-9 md:p-8">
        ...
        <div className="space-y-8 z-10">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-xl font-display font-extrabold tracking-tight">Folo<span className="text-indigo-400">Up</span> AI Portal</span>
          </div>
          ...
          <div className="space-y-4 pt-8">
            ...
            <p className="text-xs text-slate-300 leading-relaxed font-light">
              FoloUp uses conversational, intelligent AI follow-ups to evaluate job candidates through rich, natural speech analytics.
            </p>
          </div>
        </div>
                  ...
                  <div className="space-y-4 bg-indigo-50/30 border border-indigo-100/50 rounded-2xl p-5 relative overflow-hidden">
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-[11px] text-slate-500 font-light leading-normal">
                        FoloUp AI has dispatched a login magic token to <strong>{magicEmail}</strong>. Since you are evaluating in the sandbox preview mode, you can trigger passwordless validation below:
                      </p>
                    </div>

                    <div className="bg-white border border-indigo-100/40 rounded-xl p-4.5 space-y-3 shadow-xs">
                      <div className="flex justify-between items-center text-[10px] font-mono text-slate-400">
                        <span>Sender: login-noreply@foloup.ai</span>
                        <span>Just now</span>
                      </div>
                      <div className="space-y-1">
                        <p className="text-xs font-bold text-slate-800">Your FoloUp Passwordless Authentication Link</p>
